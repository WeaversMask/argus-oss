...
- Everyday commands: `pnpm test` · `pnpm lint` · `pnpm typecheck` · `pnpm gates:check`. There is no build step — the library packages' `exports` point straight at `src/`, and the CLI runs from source, so `pnpm typecheck` (`tsc --noEmit`) is what verifies the code compiles.
