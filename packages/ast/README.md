...
- **Memory:** two obligations and one hard limit. (1) `AstDocument` holds a wasm tree — `parse()` frees it internally; `parseDocument()` callers own `dispose()` (idempotent; `parsed` stays valid after). (2) `TreeSitterAstParser.dispose()` deletes the engine `Parser` and drops grammar references — but web-tree-sitter's `Language` has **no free API**, so each grammar load is wasm memory held until process exit: **one adapter instance per process** is the design; don't churn instances outside tests. Also: a retained `AstNode` pins its file's **full source string** via the lazy `text` getter — copy `text` out if you store nodes beyond a scan.
