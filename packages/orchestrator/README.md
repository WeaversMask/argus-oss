...
- **A `+++` line is only a header when the `---` partner precedes it.** The two hardenings above are one mechanism: once the backstop re-synchronises mid-body, the remaining body lines _are_ walked, and a lone `+++ ` check let a patch file's own added content rename the current target and swallow that file's later hunks. git always emits the pair adjacently, so requiring it costs nothing and closes the gap.
