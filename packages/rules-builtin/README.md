...
| `quality/no-dead-code`          | warning | —           | Statements after a `return`/`throw`/`break`/`continue` in the same block          |
| `style/naming-convention`       | warning | —           | Non-PascalCase types, non-camelCase functions, snake_case variables               |
...
- **Uncovered defensive branches (all guard against degenerate/malformed parse trees, part of the never-crash posture):** `import-order` — an `import_statement` with no `source` field; `naming-convention` — a checked declaration with no `name` field (e.g. an anonymous default export is not a `class_declaration`, so it is not reached); `no-empty-test` — a test call with no `arguments` node. Coverage sits at ~98% lines / ~96% branches.
