...
- **Sign-off gates run at the repo ROOT before every push:** `pnpm lint && pnpm typecheck && pnpm test && pnpm gates:check`. `pnpm --filter <pkg>` runs bypass turbo's task graph and miss whole-graph failures (P1-02: a workspace dependency cycle reached CI exactly this way). Filtered runs are for iteration only, never sign-off. **`pnpm typecheck` is the compile verification** — the workspace is buildless by ruling (D-5), so `tsc --noEmit` across all 10 packages is what proves the code compiles. **`pnpm build` is deliberately not in that list:** it runs **zero tasks** today (no package declares a `build` script, and turbo calls that success), so citing it as sign-off evidence asserts nothing — which it did from P0-05 to OPS-07. The command and its CI job still exist, because D-8's bundle makes them real; `pnpm gates:check` fails the day a package declares `build`, so the gate list gets updated rather than quietly going stale again.
