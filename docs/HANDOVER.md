...
- **For OPS-05 specifically:** the [go-public runbook](./go-public-runbook.md) item 7 rates the real-name display stamp on web-UI merge commits as "optional, cosmetic". `workflow.md` now actively points readers at `git log origin/main --merges` as a receipt, which makes those 52 commits a documented destination rather than an incidental one. Worth re-weighing that rating rather than inheriting it.
