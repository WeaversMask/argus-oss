...
- **P0-06 (Docker) still unstarted.** The `p0-06-docker-dev-env` branch contains **only** the P0-07 plan-insertion commit (`52b9f8e`) — no `Dockerfile.dev`, no `docker-compose.yml` yet. The full P0-06 recipe/plan is preserved in [`docs/handovers/p0-05-ci-pipeline-handover.md`](./p0-05-ci-pipeline-handover.md) (archived this session). Not part of the licensing arc; flagged only so it isn't lost.
