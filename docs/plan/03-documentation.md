...
- **README badges — the distinction is binary property vs. number, not "does it repeat the page".** Repeating what the page already says is the _point_ of a badge: a visitor scans badges in seconds and reads prose only if something earns it. What cannot be badged is an ungated **number**. A binary property can be held up by a gate — Prettier by `format:check`, strict TypeScript by the typecheck gate, Conventional Commits by commitlint — so it cannot quietly become false. A number has nothing holding it up, which is exactly how the README's demo recording came to claim 151 files against an actual 161. Nine badges went live on 2026-08-10 in three tiers, recorded in an HTML comment beside them: **live** (re-measure themselves — CI, last-commit, and node, which shields parses out of `engines.node` on `main`), **gated** (a build fails if they stop being true), and **stated** (true, but nothing in CI asserts them — License, project status, tree-sitter; re-check these by hand when the underlying fact changes). **Coverage and mutation remain absent, and adding either means doing the work, not adding a line.** Coverage needs a service (Codecov/Coveralls) wired before shields.io can read it; until then the enforced floors in [`../../vitest.config.ts`](../../vitest.config.ts) are the honest claim. Mutation has no score to publish while the weekly Stryker job is red (since 2026-07-28) — fix the job first. This is DOC-02's own precedent for keeping mutation score out of the README's receipts table.
