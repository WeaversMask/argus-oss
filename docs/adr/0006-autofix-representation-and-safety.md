...
- The four decline conditions in decision 2 are **sufficient, not provably complete** — they are the cases we found (two of them only because a reviewer went looking). A fifth shape of unsafe reorder probably exists. The mitigation is structural rather than analytical: declining is cheap and always safe, so the bar for adding a condition is deliberately low.
