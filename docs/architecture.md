...
Nine packages and one app are real; the rest of the tree in `01-repo-structure.md` is planned.
...
| [`@argus/orchestrator`](../packages/orchestrator/README.md)           | What a scan **covers**, as opposed to what it judges: diff mode turns a git base ref into the changed files and lines (merge base, working tree, untracked included) and narrows violations to them. Git arrives as an injected `GitRunner`, not a core port, so the decisions are testable without a repository ([ADR-0008](./adr/0008-scan-scope-orchestration.md)). | [`packages/orchestrator/README.md`](../packages/orchestrator/README.md)           |
