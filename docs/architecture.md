...
Seven packages and one app are real; the rest of the tree in `01-repo-structure.md` is planned.
...
| [`@argus/api-contracts`](../packages/api-contracts/README.md) | The wire format: zod schemas + inferred types for everything Argus emits to a machine — today `argus check --format json`, later the API server and web UI payloads. Depends on zod alone, deliberately **not** on core, so a consumer can adopt it without the domain. | [`packages/api-contracts/README.md`](../packages/api-contracts/README.md) |
