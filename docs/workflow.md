...
- **Four of the eleven CI jobs report without blocking.** The boundary, review, documentation-delta, and dogfooding jobs go red on failure but are not yet in the required-checks list, so they rely on the maintainer not merging a red PR. The other seven block outright. (This bullet read "three of eleven … other eight" until OPS-07: the documentation-delta job landed a day after it was written and was never counted, and the `Build` job it counted as blocking was removed for never having checked anything. A hand-maintained count of mechanisms is itself a claim that rots.)
