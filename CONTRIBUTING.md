...
5. **Keep notices current.** If your change alters the dependency tree, regenerate [THIRD-PARTY-NOTICES](THIRD-PARTY-NOTICES) with `pnpm notices` in the same PR. CI enforces this: the `license` job runs `pnpm notices:check`, which regenerates in memory and fails if the committed file no longer matches the tree. Run it locally to see what CI will see. It compares everything above the file's `PLATFORM-SPECIFIC PACKAGES` heading — packages that declare `os`/`cpu`/`libc` resolve differently on every machine, so that trailing section is inventoried but cannot be compared across hosts.
